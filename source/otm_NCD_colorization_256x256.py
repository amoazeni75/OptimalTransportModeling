...
num_inception_imgs = 60  # number of images used to compute FID
...
train_loaderA = torch.utils.data.DataLoader(train_dataA, batch_size=BATCH_SIZE, num_workers=num_workers, shuffle=True,
                                            drop_last=True)
train_loaderB = torch.utils.data.DataLoader(train_dataB, batch_size=BATCH_SIZE, num_workers=num_workers, shuffle=True,
                                            drop_last=True)
...
def Test(iteration, G, test_loader_iterator):
    ...
    save_image(x.view(x.shape[0], channels, size, size),
               output_path + 'inpaint_sample_iteration_' + str(iteration) + '.png', nrow=8, normalize=True)
    save_image(y.view(y.shape[0], channels, size, size),
               output_path + 'real_sample_iteration_' + str(iteration) + '.png', nrow=8, normalize=True)
